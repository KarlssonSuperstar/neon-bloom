...
const CASSETTE_DATA = [
  {
    ...
    cta: "Read Story", variant: "bloom", route: "RT-44 / NORTHWALL", status: "SEALED", sig: "0xA7-BLOOM",
  },
  {
    ...
    cta: "Meet the Courier", variant: "bloom", route: "OPERATIVE / FREELANCE", status: "ACTIVE", sig: "PINK-01",
  },
  {
  ...
  },
  {
    ...
    cta: "Explore Bloom", variant: "bloom", route: "LIVING NETWORK", status: "PULSING", sig: "BL-INF",
  },
  {
  ...
  },
  {
    ...
    cta: "Discover the Frontier", variant: "bloom", route: "OUTER FRONTIER", status: "UNSTABLE", sig: "OT-FAR",
  },
];
...
function Cassette({ data }: { data: typeof CASSETTE_DATA[0] }) {
  ...
  const targetFlip = isMobile ? (isExpanded ? 180 : 0) : (isHovered ? 180 : 0);
  ...
  return (
    <div 
      ...
      onClick={handleTap}
    >
      {/* Dark backdrop overlay for expanded mobile state */}
      {isMobile && (
        <motion.div
          initial={false}
      ...
      <div className="absolute bottom-2 left-0 right-0 text-center md:hidden">
        <h4 className={cn(
          ...
          isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom"
        )}>
          {data.title}
        </h4>
      </div>
      ...
      <div className="absolute top-1/2 left-1/2 -ml-[163px] -mt-[307.5px] [perspective:1400px] z-50 pointer-events-none" style={{ width: W, height: H }}>
        ...
        >
          ...
          <div className="absolute inset-0 z-10 flex flex-col items-center text-center pl-[84px] pr-[76px] pt-[140px] pb-[125px]">
            ...
            <div className={cn("font-display text-[11px] tracking-widest mb-0.5 font-bold", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
              {isCrown ? "クラウン・ディビジョン" : isSky ? "スカイ・セクター" : "ネオン・ブルーム"}
            </div>
            <div className={cn("font-mono text-[9px] tracking-[0.2em] uppercase mb-2", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
              {isCrown ? "CROWN DIVISION" : isSky ? "SKY SECTOR" : "NEON BLOOM"}
            </div>

            {/* BOTTOM PANEL */}
            <div className={cn("flex-1 w-full border flex flex-col text-left p-3 relative mt-1 bg-[#0a0c12]/30 backdrop-blur-sm", isCrown ? "border-crown/40 rounded-t-lg rounded-b-sm" : isSky ? "border-sky/40 rounded-t-lg rounded-b-sm" : "border-bloom/40 rounded-t-lg rounded-b-sm")}>
              
              <div className={cn("font-display text-[42px] font-bold leading-[0.8] mb-2 tracking-tighter", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
                {data.id.split('-')[1] || data.id}
              </div>
              
              <div className={cn("font-mono text-[8px] uppercase tracking-[0.15em] mb-1", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
                {data.label}
              </div>
              <div className={cn("font-display text-[8px] tracking-widest opacity-80 mb-2", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
                コアモジュール
              </div>
              
              {/* Fake Barcode */}
              <div className={cn("w-full h-[16px] mb-2 opacity-90", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
              ...
              <div className={cn("font-mono text-[8px] tracking-widest mb-2", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
                NB-05-{data.id}
              </div>
              
              <div className={cn("mt-auto flex flex-col gap-0.5 font-mono text-[7px] tracking-widest", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
              ...
              <div className={cn("absolute bottom-1.5 left-1.5 w-1.5 h-1.5 border-b border-l", isCrown ? "border-crown/40" : isSky ? "border-sky/40" : "border-bloom/40")} />
              <div className={cn("absolute bottom-1.5 right-1.5 w-1.5 h-1.5 border-b border-r", isCrown ? "border-crown/40" : isSky ? "border-sky/40" : "border-bloom/40")} />
            </div>
        ...
        >
          ...
          <div className="absolute inset-0 z-10 pt-[110px] pb-[115px] px-[30px] flex flex-col gap-2 text-paper font-sans">
            ...
            <div className="border border-white/10 rounded-sm p-2.5 flex gap-2 relative flex-1 bg-[#0a0c12]/50 backdrop-blur-sm">
              ...
              <div className="flex-1 flex flex-col">
                <h3 className={cn("font-mono text-[10px] tracking-widest uppercase mb-2", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>MODULE OVERVIEW</h3>
                <p className="text-[10.5px] leading-[1.6] text-white/70 mb-2">
                  {data.text}
                </p>
                <p className="text-[10.5px] leading-[1.6] text-white/70 mt-auto">
                  Handle only with verified {data.variant === "crown" ? "Crown" : "Bloom"} clearance.
                </p>
              </div>
              <div className="w-[70px] flex flex-col items-center shrink-0 border-l border-white/10 pl-2">
                <div className={cn("w-full aspect-[4/5] border p-1 mb-2 relative flex items-center justify-center", isCrown ? "border-crown/50" : isSky ? "border-sky/50" : "border-bloom/50")}>
                  <div className={cn("absolute inset-0 opacity-20", isCrown ? "bg-[repeating-linear-gradient(45deg,rgba(255,214,10,0)_0px,rgba(255,214,10,0)_4px,rgba(255,214,10,1)_4px,rgba(255,214,10,1)_8px)]" : isSky ? "bg-[repeating-linear-gradient(45deg,rgba(105,180,255,0)_0px,rgba(105,180,255,0)_4px,rgba(105,180,255,1)_4px,rgba(105,180,255,1)_8px)]" : "bg-[repeating-linear-gradient(45deg,rgba(255,61,138,0)_0px,rgba(255,61,138,0)_4px,rgba(255,61,138,1)_4px,rgba(255,61,138,1)_8px)]")} />
                  <svg viewBox="0 0 24 24" fill="currentColor" className={cn("w-8 h-8 relative z-10", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
                    {isCrown ? (
                      <path d="M12 2L15 8L22 10L16 15L18 22L12 18L6 22L8 15L2 10L9 8L12 2Z" />
                    ) : (
                      <path d="M12 2C12 2 15 7 15 11C15 15 12 18 12 18C12 18 9 15 9 11C9 7 12 2 12 2ZM12 18C12 18 17 19 20 16C23 13 22 9 22 9C22 9 19 10 16 13C14 15 12 18 12 18ZM12 18C12 18 7 19 4 16C1 13 2 9 2 9C2 9 5 10 8 13C10 15 12 18 12 18Z" />
                ...
                <div className={cn("font-mono text-[6.5px] tracking-wider text-center uppercase leading-tight mt-0.5", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>
                  {data.variant === 'bloom' ? 'NEON BLOOM' : data.variant === 'crown' ? 'CROWN DIV' : 'SKY SECTOR'}
                  <br/>
            ...
            <div className="border border-white/10 rounded-sm p-2 relative bg-[#0a0c12]/50 backdrop-blur-sm">
              <CornerAccents />
              <h3 className={cn("font-mono text-[8px] tracking-widest uppercase mb-1", isCrown ? "text-crown" : isSky ? "text-sky" : "text-bloom")}>PAYLOAD IDENTIFICATION</h3>
...
export default function Cassettes() {
  return (
    <section id="cargo" className="w-full max-w-[1480px] mx-auto px-exp-3 md:px-exp-4 pb-exp-6">
